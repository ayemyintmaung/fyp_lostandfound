...
import {getFoundItemListAPI, deleteFoundItemAPI} from '../../actions/itemlist'
...
class FoundItems extends Component {
  ...
  deleteFoundItem(id){
    // console.log(id);
    this.props.deleteFoundItem(id)
    .then(response => {
        Notification({
            title: 'Success',
            message: 'Delete Found Item Successful',
            type: 'success',
            duration: 1500
          });
  ...
  handleDeleteBtn(index) {
    ...
    this.deleteFoundItem(this.state.data[index]._id);
    const { data } = this.state;
    data.splice(index, 1);
    this.setState({
      data: [...data]
    });
    this.props.onDecreaseFoundItemCount();
  }
        ...
        return (
      	<Col className="md-12">
        	<Layout.Col span="22"  offset="1"><div className="grid-content bg-purple-dark my-detail-card" >
       			<Card className="box-card, TABLE_CARD_FOUND_ITEM">
...
const reduxDispatch = (dispatch) => ({
  getFoundItemList: () => dispatch(getFoundItemListAPI()),
  deleteFoundItem:(id)=>dispatch(deleteFoundItemAPI(id))
});
